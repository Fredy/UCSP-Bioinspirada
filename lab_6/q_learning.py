...
def q_learning(weights=None, print_info=False):
    if weights is None:
        q = defaultdict(_init)
    else:
        q = weights
    ...
    for t in itertools.count():
        ...
        for i in range(MAX_ITERATIONS):
            ...
            if print_info:
                sys.stdout.write(f'\rT: {t} - {i}/{MAX_ITERATIONS}'
                                 f' Pos: {next_state}')
                sys.stdout.flush()

            if reward == 1:
                # Agent found the end point
                done = True
                break
    ...
    total_iterations = t * MAX_ITERATIONS + i + 1
    return q, total_iterations
